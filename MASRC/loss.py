...
def bce_wo_weight(pred, label, is_reg=True):
    loss = F.binary_cross_entropy(pred, label)
    if is_reg:
        reg = torch.mean(pred * pred)
        return loss + 0.99* reg
    return loss


def pseudo_bce(pred, label):
    weight = weighter(label, [1, 1.15])
...
def match_scene(feat, label):
    ...
    bef_vec = torch.mean(feat[:, :half], dim=1)
    aft_vec = torch.mean(feat[:, half:], dim=1)
    cosin = 0.5*(1-torch.sum(bef_vec*aft_vec, dim=-1))

    loss = F.binary_cross_entropy(cosin, label)
